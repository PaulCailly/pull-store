...
const put = async (req, res, next) => {
  ...
  try {
    ...
    if (requestIdsOk) {
      const item = await service.update(req.headers.actualItem, servicesRequests);

      if (checkResponsesStatusCodes(item)) {
        res.status(200).send({
          message: `${req.headers.actualItem} successfully updated`,
          code: 1,
          serviceRes: item,
        });
      } else {
        res.status(200).send({
          message: `We can't update the ${req.headers.actualItem} in database`,
          code: 0,
          serviceRes: item,
        });
      }
